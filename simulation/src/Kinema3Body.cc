...
double Kinema3Body::ProFunction(double m12, double m1, double m2, double m3, double M)
{
  ...
  if (abs(m12-(m1+m2))<10e-20)
    m12 = m1+m2;
  else if (abs(m12-(M-m3))<10e-20)
    m12 = M-m3;
  ...
  if ((m12 >= m1+m2) && (m12 <= M-m3)) {
    ...
    fprintf(stderr, "             m1=%.8f, m2=%.8f, m3=%.8f, M=%.8f, m12=%.8f m1+m2=%.8f\n",
	    m1,m2,m3,M,m12,m1+m2);
    fprintf(stderr, "             m12-(m1+m2)=%.e\n",m12-(m1+m2));
    fprintf(stderr, "             M-m3-(m12)=%.8f\n",M-m3-m12);
    exit(1);
  }
}
