...
bool SksTrack::doFitSksIn( void )
{
  ...
  //std::cout << "start RKtrace" << std::endl;
  while( ++iIter < MaxIteraction ){
    //std::cout << "RKtrace :  " << iIter << std::endl;    
    if( !RKtrace( iniCord, hPntContSksIn_ ) ){
      // Error 
#ifdef WARNOUT 
      std::cerr << "[" << funcname << "]: Error in RKtrace. " << std::endl;
      std::cout << "[" << funcname << "]: Error in RKtrace. " << std::endl;
#endif
      break;
    }

    //int iPlane = hPntContSksIn_.size()-1;
    //int plnum=hPntContSksIn_[iPlane].first;    
    //std::cout << "hoge1 : plnum = " << plnum << std::endl;

    chiSqr = calcChiSqr( hPntContSksIn_, hitArraySksIn_);
    //chiSqr = calcChiSqr( hPntContSksIn_);
    double dChiSqr = chiSqr-prevChiSqr;
    double dChiSqrR = dChiSqr/prevChiSqr;
    double Rchisqr = dChiSqr/estDChisqr;

    //iPlane = hPntContSksIn_.size()-1;
    //plnum=hPntContSksIn_[iPlane].first;    
    //std::cout << "hoge2 : plnum = " << plnum << std::endl;
    ...
    PrintCalcHits( hPntContSksIn_, std::cout );
#endif
    ...
    //iPlane = hPntContSksIn_.size()-1;
    //plnum=hPntContSksIn_[iPlane].first;    
    //std::cout << "hoge3 : plnum = " << plnum << std::endl;


    //if( !guessNextParameters( hPntContSksIn_, iniCord, 
    //			      estDChisqr, lambdaCri, dmp ) )
    if (!guessNextParameters( hPntContSksIn_, hitArraySksIn_, iniCord, estDChisqr, lambdaCri, dmp ))
      return statusSksIn_=false;

    //iPlane = hPntContSksIn_.size()-1;
    //plnum=hPntContSksIn_[iPlane].first;    
    //std::cout << "hoge4 : plnum = " << plnum << std::endl;
    ...
    { 
      ...
      std::cout << "####:" ;
      std::cin >> ii;
      if(ii<0) return statusSksIn_;
    }
#endif

  }  /* End of Iteration */
  //std::cout << "end  RKtrace loop" << std::endl;
...
bool SksTrack::saveTrackParametersSksIn( const RKCordParameter &cp )
{
  ...
  priPosSksIn_ = geomMan.Global2LocalPos( TGTid, pos );
  priMomSksIn_ = geomMan.Global2LocalDir( TGTid, mom );
