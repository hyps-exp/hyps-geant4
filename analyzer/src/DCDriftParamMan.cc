...
double DCDriftParamMan::DriftLength6(int PlaneId, double dt, double p1, double p2 ,double p3, double p4, double p5)
{
  //For SDC1
  if (PlaneId>=1 && PlaneId<=4) {
    if (dt<-10 || dt>120) // Tight drift time selection
    ...
    if (dt<-10 || dt>220) // Tight drift time selection
    ...
    if (dt<-10 || dt>55) // Tight drift time selection
      //if (dt<-10 || dt>80) // Loose drift time selection
      return 999.9;
