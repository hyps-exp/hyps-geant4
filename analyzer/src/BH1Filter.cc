...
//______________________________________________________________________________
void
BH1Filter::Apply_withT0Seg(int T0Seg,
			   const DCAnalyzer& dc,
			   std::vector<std::vector<DCHitContainer> >& candidates)
{
  // +++++++++++++++++++++++++++++++++++++++++++
...
//______________________________________________________________________________
void BH1Filter::BuildCandidates(std::set<int> & seg,
				std::vector<std::vector<DCHitContainer> >& candidates)
{
...
//       std::cout << "  BH1 seg = " << iSeg << "\n";
      for (int iplane=0; iplane<NumOfLayersBcIn; ++iplane)
	{
	  ...
	  int iLayer = iplane + 1;
	  const DCHitContainer& before = m_dc->GetBcInHC(iLayer);
...
//  			<< " (" << xmin << ", " << xmax << ")";
		...
		{
		  //		  std::cout << " layer = " << iplane << std::endl;
		  continue;
		}
