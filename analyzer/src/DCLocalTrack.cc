...
double DCLocalTrack::GetFTOFX( ) const
{
  int IdTof = DCGeomMan::GetInstance().GetTofId();
  double z0 = DCGeomMan::GetInstance().GetLocalZ( IdTof );
  double RA2= DCGeomMan::GetInstance().GetRotAngle2( IdTof );

  double z = (x0_+z0*tan(RA2*Deg2Rad))/(tan(RA2*Deg2Rad)-u0_);
  /*
  double x = (x0_+u0_*z);
  double y = (y0_+v0_*z);

  std::cout << "(x, y, z) = (" << 2207.+(z-z0) << ", " << x-150 << ", " << y << ")" << std::endl;
  */

  return (x0_+u0_*z)/sin(RA2*Deg2Rad);

}

double DCLocalTrack::GetFTOFY( ) const
{
  int IdTof = DCGeomMan::GetInstance().GetTofId();
  double z0 = DCGeomMan::GetInstance().GetLocalZ( IdTof );
  double RA2= DCGeomMan::GetInstance().GetRotAngle2( IdTof );
