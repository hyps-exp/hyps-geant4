...
SksTrack::SksTrack( DCLocalTrack *In, DCLocalTrack *Out )
  : In_(In), Out_(Out), iniP_(0.), // iniP_(DefPini), 
...
bool SksTrack::doFit( void )
{
  ...
  */
  double xTof = Out_->GetFTOFX();
  double yTof = Out_->GetFTOFY();
  ...
  */
  /*For FTOF Rotation angle*/
  int lnum = DCGeomMan::GetInstance().GetDetectorId("SDC4-x-2");
  ThreeVector gmTof = DCGeomMan::GetInstance().
    Local2GlobalDir( lnum, ThreeVector( pz*uTof, pz*vTof, pz ) );

  ThreeVector gpSDC4 = DCGeomMan::GetInstance().
    Local2GlobalPos( lnum, ThreeVector( 0, 0, 0.0 ) );
  ...
  if(iniP_>0){
    RKCordParameter::RKSetPolarity(-1.);    
    // RKcordparameter iniCord_neg(gpTof, pz*uTof, pz*vTof, 1.0/gmTof.mag());
    // iniCord = iniCord_neg;
  }else{
    RKCordParameter::RKSetPolarity(1.);
    // RKCordParameter iniCord_pos(gpTof, pz*uTof, pz*vTof, -1.0/gmTof.mag());
    // iniCord = iniCord_pos;
  }
...
bool SksTrack::saveTrackParameters( const RKCordParameter &cp )
{
  ...
  // polarity_ = priMom_.z()<0. ? -1. : 1.;
  polarity_ = iniP_<0. ? -1. : 1.;
